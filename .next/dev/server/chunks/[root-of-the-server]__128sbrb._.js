...
"[project]/node_modules/tree-sitter/index.js [app-route] (ecmascript)", ((__turbopack_context__, module, exports) => {

const binding = typeof process.versions.bun === "string" ? (()=>{
    throw new Error('could not resolve "' + `./prebuilds/${process.platform}-${process.arch}/tree-sitter.node` + '" into a module');
})() : __turbopack_context__.r("[project]/node_modules/node-gyp-build/index.js [app-route] (ecmascript)")(("TURBOPACK compile-time value", "/ROOT/node_modules/tree-sitter"));
...
Query.prototype.matches = function(node, { startPosition: startPosition1 = ZERO_POINT, endPosition: endPosition1 = ZERO_POINT, startIndex = 0, endIndex = 0, matchLimit = 0xFFFFFFFF, maxStartDepth = 0xFFFFFFFF, timeoutMicros = 0 } = {}) {
    marshalNode(node);
    const [returnedMatches, returnedNodes] = _matches.call(this, node.tree, startPosition1.row, startPosition1.column, endPosition1.row, endPosition1.column, startIndex, endIndex, matchLimit, maxStartDepth, timeoutMicros);
...
Query.prototype.captures = function(node, { startPosition: startPosition1 = ZERO_POINT, endPosition: endPosition1 = ZERO_POINT, startIndex = 0, endIndex = 0, matchLimit = 0xFFFFFFFF, maxStartDepth = 0xFFFFFFFF, timeoutMicros = 0 } = {}) {
    marshalNode(node);
    const [returnedMatches, returnedNodes] = _captures.call(this, node.tree, startPosition1.row, startPosition1.column, endPosition1.row, endPosition1.column, startIndex, endIndex, matchLimit, maxStartDepth, timeoutMicros);
    const nodes = unmarshalNodes(returnedNodes, node.tree);
    const results = [];
    let i = 0;
    let nodeIndex = 0;
...
function unmarshalNode(value, tree, offset = 0, cache = null) {
    /* case 1: node from the tree cache */ if (typeof value === 'object') {
        const node = value;
        return node;
    }
    /* case 2: node being transferred */ const nodeTypeId = value;
